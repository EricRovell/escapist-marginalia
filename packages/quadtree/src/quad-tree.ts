...
export class QuadTree<Item extends Point = Point> {
	...
	/**
	 * Returns the the boundary's origin (top-left vertex) `x` coordinate.
	 */
	get x(): number {
	...
	/**
	 * Returns the the boundary's origin (top-left vertex) `y` coordinate.
	 */
	get y(): number {
	 ...
	 */
	query(range: Circle | Rectangle, items: Item[] = []): Item[] {
