---
{
	...
	"dateUpdated": "2024-03-20T14:04:00.000Z",
...
To look at a relatively small table, find the right date and to check the time is not a particularly difficult task. Nevertheless, I had an urge to simplify the task for myself and my colleagues by creating a minimalistic web-application to display the city lighting schedule inmorer meaningful way.
...
In this project I used `workspaces` for the first time to more conveniently break the application into individual parts. Perhaps it was possible to do without them, but this made it possible to organize the codebase extremely conveniently and increase the convenience of reusing code between different parts.

### Versioning `localStorage`

`localStorage` is used to store application settings and the settings schema may change. It was necessary to provide versioning for the application to work correctly when schema is going to change. To do this, I learning about how versioning works and wrote a separate package for working with versioned `localStorage`.
...
Thanks to the project, I implemented full-fledged [PWA][pwa] functionality, used [`service-worker`][service-worker] for the first time and was extremely delighted with its capabilities.
